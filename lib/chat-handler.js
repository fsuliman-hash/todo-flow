...
function sanitizeTaskActionPayload(task) {
  ...
  const category = ACTION_CATEGORY_KEYS.has(categoryRaw) ? categoryRaw : 'personal';
...
async function handleChatActionPayload(body, env) {
  ...
  const cleaned = sanitizeTaskActionPayload(task);
  if (!cleaned) {
...
function sanitizeChatPlanActions(rawActions, opts) {
  ...
  arr.slice(0, cap).forEach((a) => {
    ...
    const risk = ['low', 'medium', 'high'].includes(String(a.risk || '').toLowerCase())
      ...
      : 'medium';
    if (type === 'task.create') {
      const task = sanitizeTaskActionPayload(a.task || a.payload || {});
      if (!task) return;
    ...
    if (type === 'task.update') {
      ...
      if (!edit || !edit.patches || typeof edit.patches !== 'object') return;
      out.push({
        type,
        risk,
        edit: {
          action: 'update',
          taskId: String(edit.taskId || '').trim(),
          matchTitle: String(edit.matchTitle || '').trim(),
    ...
    if (type === 'task.bulk_update') {
      ...
      const cleaned = updates
        .slice(0, cap)
        .map((u) => {
          if (!u || typeof u !== 'object' || !u.patches || typeof u.patches !== 'object') return null;
          return {
            action: 'update',
            taskId: String(u.taskId || '').trim(),
            matchTitle: String(u.matchTitle || '').trim(),
            patches: u.patches,
          };
        })
        .filter(Boolean);
...
async function handleChatPlanPayload(body, env) {
  ...
  const actions = sanitizeChatPlanActions(parsed.actions, { actionCap: body?.actionCap });
  const assistantReply = String(parsed.assistantReply || '').trim().slice(0, 500);
  return { status: 200, json: { plan: { actions, assistantReply }, model: result.model } };
}
