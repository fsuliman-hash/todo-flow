...
function rHdr(title,sub){
  const active=R.filter(r=>!r.completed&&isTaskStartVisible(r)),ov=active.filter(r=>urg(r.dueDate)==="overdue").length,sc=active.filter(r=>{const u=urg(r.dueDate);return u==="urgent"||u==="soon"}).length;
...
function rTasks(){
  ...
  const activeTasks=R.filter(r=>!r.completed&&isTaskStartVisible(r));
  const dueOverdueCount=activeTasks.filter(r=>urg(r.dueDate)==='overdue').length;
  const dueSoonCount=activeTasks.filter(r=>['urgent','soon'].includes(urg(r.dueDate))).length;
  const dueLaterCount=activeTasks.filter(r=>urg(r.dueDate)==='later').length;
  const filteredTasks=getFiltered();
  h+=`<div class="task-filter-head"><div class="task-filter-label">Due filters</div><div class="task-filter-actions"><button class="chip-btn${filter==='overdue'?' on':''}" onclick="filter='overdue';render()">Overdue (${dueOverdueCount})</button><button class="chip-btn${filter==='duesoon'?' on':''}" onclick="filter='duesoon';render()">Due Soon (${dueSoonCount})</button><button class="chip-btn${filter==='duelater'?' on':''}" onclick="filter='duelater';render()">Due Later (${dueLaterCount})</button><button class="chip-btn${filter==='all'?' on':''}" onclick="filter='all';render()">Total (${activeTasks.length})</button></div></div>`;
