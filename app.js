...
function createMaintenanceReminder(kind='maintenance'){
  ...
  openRecordModal({title:kind==='seasonal'?'Seasonal task':'Home maintenance task',subtitle:'Create a reminder directly from the home module.',submitLabel:'Save task',fields:[
...
function addSeasonalReminder(id=''){
  ...
  openRecordModal({title:existing?'Edit seasonal reminder':'Add seasonal reminder',submitLabel:existing?'Save reminder':'Add reminder',fields:[
...
function addShoppingTemplate(){
  ...
  ],onSubmit:(vals)=>{const name=vals.name.trim();if(!name)return false;const items=(vals.items||'').split(',').map(x=>x.trim()).filter(Boolean).map(text=>({id:gid(),text,done:false}));X.shoppingLists.push({id:gid(),name,template:true,items});sv();render();showToast('Shopping template saved');return true;}
  });
}
...
function openChoreForm(id=''){
  ...
  openRecordModal({title:chore?'Edit chore':'Add chore',submitLabel:chore?'Save chore':'Add chore',fields:[
  ...
  ],onSubmit:(vals)=>{const title=vals.title.trim();if(!title)return false;const rec=chore||{id:gid(),lastDone:'',lastAssigned:'',assigneeByWeek:{}};Object.assign(rec,{title,points:Math.max(1,Number(vals.points)||1),rotation:vals.rotation||'weekly'});if(!chore)X.chores.unshift(rec);sv();render();showToast(chore?'Chore updated':'Chore added');return true;}
  });
}
  ...
  ov.innerHTML=`<div class="mo-in shift-modal" onclick="event.stopPropagation()">
    ...
    <div class="shift-editor-panel">
      ...
      <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">
        <button class="sbtn" style="flex:1;min-width:180px" onclick="saveShift('${shiftModalDate}')">${editing?"Update shift":"Save shift"}</button>
...
function openTemplateForm(id){
  ...
  ov.innerHTML=`<div class="mo-in" onclick="event.stopPropagation()"><div class="mo-h"></div><h3>${t?"Edit":"New"} Template</h3>
    ...
    <button class="sbtn" onclick="saveTemplateForm()" style="margin-top:12px">${t?"Save":"Create Template"}</button></div>`;
  document.body.appendChild(ov);
  renderTemplateItems();
...
function openDispatchForm(){
  ...
  ov.innerHTML=`<div class="mo-in" onclick="event.stopPropagation()"><div class="mo-h"></div><h3>📞 New Dispatch</h3>
    ...
    <button class="sbtn" onclick="saveDispatch()" style="margin-top:12px">Save Dispatch</button></div>`;
  document.body.appendChild(ov);
...
function rMatrix(){
  const active=R.filter(r=>!r.completed&&(!r.startDate||new Date(r.startDate)<=new Date()));
...
function rTasks(){
  ...
  h+=`<div style="padding:8px 14px 0"><input id="searchIn" value="${esc(search)}" placeholder="Search title, notes, tags, or subject" style="width:100%;padding:10px 12px;font-size:13px;border:1.5px solid var(--border);border-radius:12px;background:var(--card);outline:none" oninput="queueSearch(this.value)"></div>`;
  ...
  h+=`<div class="task-desktop-grid"><div class="task-main-col"><div class="clist task-stack" id="taskList">${rCards(getFiltered())}</div></div>${rTaskWorkspaceAside()}</div>`;return h;
}
...
function addTaskCategory(){
  openRecordModal({title:'Add task category',submitLabel:'Save category',fields:[
    {name:'label',label:'Category name',value:'',required:true,placeholder:'Example: Mosque'},
    {name:'icon',label:'Emoji / icon',value:'🏷️',placeholder:'One emoji'},
...
function editTaskCategory(key){
  ...
  openRecordModal({title:`Edit ${cat.label}`,submitLabel:'Save category',fields:[
    {name:'label',label:'Category name',value:cat.label,required:true},
    {name:'icon',label:'Emoji / icon',value:cat.icon||'🏷️'},
...
function saveTaskCategory(existingKey,vals){
  ...
  sv();
  render();
  showToast(existingKey?'Category updated':'Category added');
  return true;
}
function deleteTaskCategory(key){
  ...
  openRecordModal({title:`Delete ${cat.label}?`,subtitle:usage?`Move ${usage} task${usage===1?'':'s'} to another category first.`:'This category is not being used right now.',submitLabel:'Delete category',fields:[
...
function logExpense(){
  ...
  openRecordModal({title:'Log expense',submitLabel:'Save expense',fields:[
...
function addShoppingItem(){openRecordModal({title:'Add shopping item',submitLabel:'Add item',fields:[{name:'text',label:'Item',value:'',required:true}],onSubmit:(vals)=>{const text=vals.text.trim();if(!text)return false;X.shoppingLists[0].items.push({id:gid(),text,done:false});sv();render();return true;}})}
...
function quickAddBill(){
  ...
  openRecordModal({title:'Add a bill',submitLabel:'Save bill',fields:[
...
function setPayAnchor(){openRecordModal({title:'Set pay-period anchor',subtitle:'Choose the first day of the biweekly period, not the payday.',submitLabel:'Save anchor',fields:[{name:'anchor',label:'Anchor date',type:'date',value:X.payPeriodAnchor||'2026-01-01',required:true}],onSubmit:(vals)=>{const d=vals.anchor||'';if(!/^\d{4}-\d{2}-\d{2}$/.test(d)){showToast('Choose a valid date');return false;}X.payPeriodAnchor=d;sv();document.querySelectorAll('.mo').forEach(m=>m.remove());render();const info=getPayCycleInfo(new Date());showToast(`Anchor saved · ${fmtLD(info.currentStart)} → ${fmtLD(info.currentEnd)}`);return true;}})}
